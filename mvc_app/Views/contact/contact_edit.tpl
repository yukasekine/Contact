        ...
        <h1>お問い合わせ内容の編集<h1>
                <form action="/contact/update/{$inquiry.id}" method="post">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="name" value="{$inquiry.name|escape}">
                        <p class="error-text">{$errorMessages['name'|default:'']}</p>
                    </div>

                    <div class="form-group">
                        <label for="kana">フリガナ</label>
                        <input type="text" class="form-control" name="kana" value="{$inquiry.kana|escape}">
                        <p class="error-text">{$errorMessages['kana'|default:'']}</p>
                    </div>

                    <div class="form-group">
                        <label for="tel">電話</label>
                        <input type="text" class="form-control" name="tel" value="{$inquiry.tel|escape}">
                        <p class="error-text">{$errorMessages['tel'|default:'']}</p>
                    </div>

                    <div class="form-group">
                        <label for="email">メールアドレス</label>
                        <input type="email" class="form-control" name="email" value="{$inquiry.email|escape}">
                        <p class="error-text">{$errorMessages['email']|default:''}</p>
                    </div>

                    <div class="form-group">
                        <label for="body">お問い合わせ内容</label>
                        <textarea class="form-control" name="body" rows="5">{$inquiry.body|escape}</textarea>
